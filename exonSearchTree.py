...
class ESTChrNode:
	...
	def add_node(self, start, end, name, pos_in_gene):
		start_chunk = int(start/self.chunk_size)	#KRC 1/24/2024 fix TypeError: 'float' object cannot be interpreted as an integer
		end_chunk = int(end/self.chunk_size)			#KRC 1/24/2024 fix TypeError: 'float' object cannot be interpreted as an integer
	...
	def find_exon(self, pos):
		chunk = int(pos/self.chunk_size) #KRC 1/24/2024 no error but still need int
		try:
